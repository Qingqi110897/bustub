...
// 检查是否存在过滤条件，并且该过滤条件是否为 ComparisonExpression 类型。
// 进一步检查该比较表达式是否是 等值比较（ComparisonType::Equal），即查询计划中有一个 "列 = 常量" 的过滤条件。
      ...
      if(comp_expr!=nullptr&&comp_expr->comp_type_==ComparisonType::Equal)
      {
        ...
        if(column_expr!=nullptr)
        {
          ...
          if (pred_key != nullptr && match_index.has_value()) {
            ...
            return std::make_shared<IndexScanPlanNode>(optimized_plan->output_schema_, table_id, index_id, filter_expr, pred_key);
          }
        }      
      }
    }
